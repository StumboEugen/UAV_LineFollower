    ...
    for pic in pics:
        ...
        if lines is not None:
            ...
            for pair in lines1[:]:
                ...
                for dir in lineInfos.keys():

                    if abs(abs(dir - theta) - np.pi) < 0.3:
                        theta = dir
                        rho = -rho

                    if abs(dir - theta) < 0.3:
                        lineInfos[dir][0] = min(lineInfos[dir][0], rho)
                        lineInfos[dir][1] = max(lineInfos[dir][1], rho)
                        dirHasRecorded = True
                        break
            ...
            if len(lineInfos) == 1:
                ...
                x0 = int(ans[0, 0] + ans[0, 1])
                y0 = int(ans[1, 0] + ans[1, 1])
